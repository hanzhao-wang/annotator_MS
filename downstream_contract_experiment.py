...
def main(
    ...
    for entry in summaries:
        ...
        # Clean benchmark: keep training data uncorrupted to compare against simulated noise.
        clean_base = Path("statdata") / "simulated" / f"{cfg.name}-{monitor}-{contract}-clean"
        clean_train_dir = clean_base / "train"
        clean_eval_dir = clean_base / "eval"
        clean_train_dir.parent.mkdir(parents=True, exist_ok=True)
        train_ds.save_to_disk(clean_train_dir.as_posix())
        ...
        full_train_dir.parent.mkdir(parents=True, exist_ok=True)
        full_corrupted_train_ds.save_to_disk(full_train_dir.as_posix())
