...
def get_args():
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="path to model"
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use LoRA to load the model",
    )
...
def main():
    ...
    # ! add n_label=1 to model_kwargs for custom classifier
    ...
    if args.lora:
        ...
        model = PeftModel.from_pretrained(model, args.model).merge_and_unload()
    ...
    if reward_pipe.tokenizer.pad_token_id is None:
        reward_pipe.model.config.pad_token_id = (
            reward_pipe.tokenizer.eos_token_id
        )
        reward_pipe.tokenizer.pad_token_id = reward_pipe.tokenizer.eos_token_id
    # For models whose config did not contains `pad_token_id`
    if reward_pipe.model.config.pad_token_id is None:
        reward_pipe.model.config.pad_token_id = (
            reward_pipe.tokenizer.pad_token_id
        )
        ...
        for step, batch in enumerate(tqdm(dataloader, desc="RM batch steps")):
            ...
            if model_type == "Custom Classifier":
                ...
                if isinstance(rewards_chosen[0], dict):
                    score_chosen_batch = [
                        result["score"] for result in rewards_chosen
                    ]
    ...
    results_leaderboard["mean"] = np.mean(list(results_leaderboard.values()))

    save_path = f"statdata/{args.model.split('/')[-2].replace('/', '_')}_rewardbench.pkl"
    with open(save_path, "wb") as f:
        pickle.dump(results_leaderboard, f)

    abstract_path = f"statdata/reward_bench_abstract.txt"
    with open(abstract_path, "a") as f:
        f.write(f"{args.model.split('/')[-2]}: {results_leaderboard}\n")
